...
An adjoint root-string alternative is now partially formalized: an oriented simple-root edge yields the two-dimensional standard sl₂ module without a fundamental highest-weight representation. Actual one-parameter adjoint curves, SU(2) generation, invariant Haar pushforward, representative coefficients, and the full radial tower are checked from an explicit compact-real coordinate certificate. Existence of that certificate for every compact simple group remains open, so this does not yet remove L02, L06, or Z05 from the completed main-proof dependency path. For rank one, the witness descends to SU(2)/{±1}; identifying a general rank-one adjoint form with that quotient remains open. Exact missing targets are type-checked in [scripts/CheckAdjointRoute.lean](scripts/CheckAdjointRoute.lean).
