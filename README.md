...
The adjoint root-string alternative now proves the full marker tower and Mathieu failure for every compact group with simple real Lie algebra and Cartan rank at least two. Complex simplicity, compact-real root normalization, and the global coordinate certificate are constructed from the actual group's Haar-averaged invariant form. The proof uses neither a fundamental highest-weight representation (L02), a root-subgroup integration theorem (L06), nor a compact simply connected covering group (Z05). Those manuscript ingredients remain unformalized. Rank one still requires identifying the adjoint form with SU(2)/{±1}; the witness on that quotient is already proved. Consequently the full nonabelian theorem and classification remain open. The proved bridge targets and exact remaining rank-one target are checked in [scripts/CheckAdjointRoute.lean](scripts/CheckAdjointRoute.lean).
